...
function initFlipNum(el){
  ...
  flapTop.className = 'fn-flap fn-flap-top';
  const flapTopInner = document.createElement('span');
  flapTopInner.className = 'fn-flap-inner';
  flapTopInner.textContent = text;
  flapTop.appendChild(flapTopInner);
  const flapBottom = document.createElement('span');
  flapBottom.className = 'fn-flap fn-flap-bottom';
  const flapBottomInner = document.createElement('span');
  flapBottomInner.className = 'fn-flap-inner';
  flapBottomInner.textContent = text;
  flapBottom.appendChild(flapBottomInner);
  el.appendChild(face);
...
function runFlip(el, newText){
  ...
  const flapTopInner = el.querySelector('.fn-flap-top .fn-flap-inner');
  const flapBottomInner = el.querySelector('.fn-flap-bottom .fn-flap-inner');
  if (!face || !flapTopInner || !flapBottomInner) return;
  const current = face.textContent;
  const next = (newText === undefined || newText === null) ? current : String(newText);
  if (next === current && newText !== undefined && el._flipQueue === undefined){
    // valeur inchangée explicitement redemandée (rare) : on rejoue quand même
    // le tap pour le retour tactile, sans dupliquer de logique plus haut.
  }
  const reduceMotion = window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  if (reduceMotion){ face.textContent = next; return; }
  flapTopInner.textContent = current;
  flapBottomInner.textContent = next;
  el.classList.remove('is-flipped');
  void el.offsetWidth; // reflow — permet de rejouer l'animation à coups répétés
  el.classList.add('is-flipped');
  el._flipping = true;
  const bottomFlap = el.querySelector('.fn-flap-bottom');
  const onEnd = (e) => {
    if (e.target !== bottomFlap) return;
    el.classList.remove('is-flipped');
...
/**
 * Compte à rebours jusqu'au coucher de soleil — lecture façon complication
 * de chronographe (H:MM), recalculée toutes les 30s. Passé l'heure du
 * coucher (soirée), affiche "—" plutôt qu'un compte à rebours négatif :
 * le readout ne ment jamais sur ce qu'il montre.
...
function startSunsetCountdown(sunsetISO){
  ...
  function tick(){
    const diffMs = sunset - new Date();
    if (diffMs <= 0){ runFlip(el, '—'); return; }
    const totalMin = Math.floor(diffMs / 60000);
    const h = Math.floor(totalMin / 60);
    const m = totalMin % 60;
    runFlip(el, h + ':' + String(m).padStart(2, '0'));
  }
  tick();
