...
function copyValue(row){
  ...
  if (!valueEl) return;
  // Si la valeur a été préparée en flip-num (voir initFlipNum), le texte
  // affiché au repos vit dans .fn-face — le conteneur porte aussi les
  // deux demi-palettes cachées, dont le texte ne doit pas être copié.
  const faceEl = valueEl.querySelector('.fn-face');
  const text = (faceEl ? faceEl.textContent : valueEl.textContent).trim();
...
/**
 * ================================================================
 * GESTE SIGNATURE — flip mécanique façon compteur.
 * Un seul geste, répété sur toute donnée technique de l'app (heures,
 * codes, températures, numéro de série, repères de plan...), à la
 * place du fade générique. Deux usages :
 *  - tap direct : initFlipNumbers() attache l'écoute sur tout élément
 *    marqué data-flip-tap, qui rejoue la bascule sur sa propre valeur.
 *  - mise à jour réelle d'une donnée (météo, compte à rebours, index
 *    de repère) : appeler runFlip(el, nouvelleValeur) plutôt que
 *    d'assigner .textContent directement, pour que le changement lui
 *    aussi bascule au lieu de s'écraser instantanément.
 * ================================================================
 */
function initFlipNum(el){
  if (!el || el._flipInit) return;
  el._flipInit = true;
  const text = el.textContent.trim();
  el.textContent = '';
  el.classList.add('flip-num');
  const face = document.createElement('span');
  face.className = 'fn-face';
  face.textContent = text;

  // Quatre demi-palettes plutôt que deux : chaque moitié (haut/bas) a une
  // version "old" (valeur qui part) et une version "new" (valeur qui
  // arrive), pour que les deux moitiés restent visibles en permanence
  // pendant la bascule — jamais de trou ni de "pop" final.
  const mk = (cls) => {
    const wrap = document.createElement('span');
    wrap.className = `fn-flap ${cls}`;
    const inner = document.createElement('span');
    inner.className = 'fn-flap-inner';
    inner.textContent = text;
    wrap.appendChild(inner);
    return wrap;
  };
  const flapTopOld = mk('fn-top-old');
  const flapTopNew = mk('fn-top-new');
  const flapBottomOld = mk('fn-bottom-old');
  const flapBottomNew = mk('fn-bottom-new');

  el.appendChild(face);
  el.appendChild(flapTopOld);
  el.appendChild(flapTopNew);
  el.appendChild(flapBottomOld);
  el.appendChild(flapBottomNew);
}

/**
 * Joue la bascule mécanique sur un élément préparé par initFlipNum().
 * Sans deuxième argument, rejoue simplement la valeur actuelle (geste
 * de tap) ; avec un texte fourni, fait basculer l'ancienne valeur vers
 * la nouvelle (mise à jour de donnée réelle). Les appels rapprochés se
 * mettent en file plutôt que de se chevaucher.
 */
function runFlip(el, newText){
  if (!el) return;
  initFlipNum(el);
  if (el._flipping){ el._flipQueue = (newText === undefined) ? el._flipQueue : newText; return; }
  const face = el.querySelector('.fn-face');
  const topOldInner = el.querySelector('.fn-top-old .fn-flap-inner');
  const topNewInner = el.querySelector('.fn-top-new .fn-flap-inner');
  const bottomOldInner = el.querySelector('.fn-bottom-old .fn-flap-inner');
  const bottomNewInner = el.querySelector('.fn-bottom-new .fn-flap-inner');
  if (!face || !topOldInner || !topNewInner || !bottomOldInner || !bottomNewInner) return;
  const current = face.textContent;
  const next = (newText === undefined || newText === null) ? current : String(newText);
  const reduceMotion = window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  if (reduceMotion){ face.textContent = next; return; }
  // Moitié haute : l'ancienne valeur bascule et disparaît à mi-course,
  // la nouvelle est déjà en place dessous, révélée au même instant.
  // Moitié basse : symétrique — l'ancienne reste visible (statique)
  // jusqu'à mi-course, puis la nouvelle bascule par-dessus. Les deux
  // moitiés sont donc toujours pleines, jamais de trou ni de saut final.
  topOldInner.textContent = current;
  topNewInner.textContent = next;
  bottomOldInner.textContent = current;
  bottomNewInner.textContent = next;
  el.classList.remove('is-flipped');
  void el.offsetWidth; // reflow — permet de rejouer l'animation à coups répétés
  el.classList.add('is-flipped');
  el._flipping = true;
  const bottomNewFlap = el.querySelector('.fn-bottom-new');
  const onEnd = (e) => {
    if (e.target !== bottomNewFlap) return;
    el.classList.remove('is-flipped');
    face.textContent = next;
    el._flipping = false;
    bottomNewFlap.removeEventListener('animationend', onEnd);
    if (el._flipQueue !== undefined){
      const queued = el._flipQueue;
      el._flipQueue = undefined;
      runFlip(el, queued);
    }
  };
  bottomNewFlap.addEventListener('animationend', onEnd);
}

/**
 * Attache le geste de tap à toute donnée technique marquée
 * data-flip-tap dans le HTML (codes, heures, températures, numéro de
 * série...). Certaines sont déjà dans une rangée cliquable (copie,
 * accordéon, appel tel:) — le tap sur le chiffre déclenche la bascule
 * en plus du comportement existant, sans le remplacer.
 * Usage : appeler une fois au chargement de chaque écran.
 */
function initFlipNumbers(){
  document.querySelectorAll('[data-flip-tap]').forEach(el => {
    initFlipNum(el);
...
/**
 * Diagramme technique "La Villa" — repères numérotés sur l'élévation au
 * trait. Chaque repère met à jour le panneau de détail en dessous, dans
 * le même geste qu'un plan annoté plutôt qu'une galerie qu'on feuillette.
 */
const TECH_PINS = {
  1: { title: 'Piscine', desc: 'Bassin à débordement chauffable, profondeur progressive jusqu\u2019à 2,1\u00A0m — éclairage nocturne inclus, ouvert de 8h à 23h.' },
  2: { title: 'Terrasse couverte', desc: 'Pergola en bois et pierre, table pour huit convives — le lieu des dîners qui s\u2019étirent, à l\u2019abri du soleil de midi.' },
  3: { title: 'Chambre principale', desc: 'Suite avec balcon privé donnant sur la mer, dressing et salle de bain attenante.' },
  4: { title: 'Vue mer', desc: 'Les îles d\u2019Hyères se dessinent à l\u2019horizon — la lumière est la plus belle au lever du soleil.' },
  5: { title: 'Accès privé', desc: 'Portail piéton à digicode ; ouverture automatique pour les véhicules sur simple demande à la conciergerie.' },
  6: { title: 'Salon', desc: 'Pièce à vivre ouverte sur la terrasse, baies vitrées coulissantes toute hauteur.' }
};
function selectTechPin(el){
  const id = el.getAttribute('data-pin');
  const data = TECH_PINS[id];
  if (!data) return;
  el.closest('svg').querySelectorAll('.tech-pin').forEach(p => p.classList.remove('active'));
  el.classList.add('active');
  const panel = document.getElementById('techPanel');
  if (panel){
    runFlip(panel.querySelector('.tp-idx'), '0' + id + ' / 06');
    panel.querySelector('.tp-title').textContent = data.title;
    panel.querySelector('.tp-desc').textContent = data.desc;
  }
  if (navigator.vibrate) navigator.vibrate(5);
}
...
/**
 * Numéro de pièce du séjour — chaque séjour devient une édition numérotée,
 * à la manière d'un certificat d'authenticité. Le numéro est dérivé de
 * façon déterministe des informations du voyageur (prénom + dates) : deux
 * visites du même voyageur affichent donc toujours le même numéro, sans
 * nécessiter de compteur côté serveur. Sans identification (aperçu par
 * défaut de la villa), on affiche l'édition de référence "0032" — celle
 * citée dans la maquette du concept. Plage : 0001–0999.
 * Usage : getStayNumber(guest) → "0032".
 */
function getStayNumber(guest){
  if (!guest || !guest.firstName || !guest.arrival || !guest.departure) return '0032';
  const seed = (guest.firstName.trim().toLowerCase() + guest.arrival + guest.departure);
  let hash = 0;
...
/**
 * ================================================================
 * CARTE-CERTIFICAT DE SÉJOUR — capture, téléchargement, partage.
 * L'écran Départ affiche une carte-certificat (numéro de série + dates
 * + mot de Stéphane) identique à la pièce montrée à l'Accueil. Ces
 * fonctions la capturent telle quelle en image PNG haute résolution,
 * pour laisser une trace tangible du séjour — téléchargeable ou
 * partageable via la feuille de partage native.
 * ================================================================ */

/**
 * Charge html2canvas à la demande, uniquement quand le voyageur déclenche
 * un téléchargement ou un partage — seul l'écran Départ en a besoin, pas
 * la peine d'alourdir le chargement des six autres écrans pour ça.
 */
function loadHtml2Canvas(){
  ...
  window._html2canvasLoading = new Promise((resolve, reject) => {
    ...
    script.onerror = () => reject(new Error('html2canvas indisponible'));
...
/**
 * Capture un élément DOM (la carte-certificat) en image PNG ×3
 * (équivalent Retina), sur fond ink plein — jamais de transparence,
 * pour un rendu fidèle une fois l'image sortie de l'app (partage,
 * pellicule photo, impression).
 * Usage : captureCertificate(document.getElementById('certCapture')).
 */
function captureCertificate(el){
  return loadHtml2Canvas().then(html2canvas => html2canvas(el, {
...
/**
 * Déclenche le téléchargement d'un canvas sous forme de fichier PNG.
 */
...
/**
 * Partage un canvas via la feuille de partage native (Web Share API,
 * niveau fichiers) si l'appareil l'expose ; repli silencieux vers le
 * téléchargement direct sinon.
 */
function shareCanvasAsImage(canvas, filename, shareTitle){
  canvas.toBlob(blob => {
    if (!blob) return;
...
function initWeather(lat, lon){
  ...
  const url = `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&current=temperature_2m,weather_code&daily=sunset&timezone=auto`;
  ...
  fetch(url, controller ? { signal: controller.signal } : undefined)
    ...
    .then(data => {
      ...
      if (label) runFlip(label, temp + '°');
      chip.classList.add('live');

      // Bandeau readout — température extérieure (même valeur que la puce,
      // dupliquée ici pour la lecture en bandeau) + compte à rebours coucher.
      const outTempEl = document.getElementById('roOutTemp');
      if (outTempEl) runFlip(outTempEl, temp + '°');

      const sunsetISO = data.daily && data.daily.sunset && data.daily.sunset[0];
      if (sunsetISO) startSunsetCountdown(sunsetISO);
    })
    .catch(() => {
...
/**
 * Heure du coucher de soleil du jour, au format HH:MM (heure locale de
 * la propriété, déjà résolue côté API via timezone=auto). Affichage
 * direct plutôt qu'un compte à rebours : plus lisible en un coup d'œil
 * pour un voyageur, et cohérent avec les autres lectures du bandeau
 * (température, etc.) qui sont toutes des valeurs, pas des durées.
 * Usage : startSunsetCountdown('2026-08-12T20:47').
 */
function startSunsetCountdown(sunsetISO){
  const el = document.getElementById('roSunset');
  if (!el) return;
  const sunset = new Date(sunsetISO);
  if (isNaN(sunset)) return;

  function tick(){
    const h = String(sunset.getHours()).padStart(2, '0');
    const m = String(sunset.getMinutes()).padStart(2, '0');
    runFlip(el, h + ':' + m);
  }
  tick();
}
...
function initAssistant(){
  ...
  fab.innerHTML = '<svg viewBox="0 0 100 100" fill="none" stroke="currentColor" stroke-width="7" stroke-linecap="round" stroke-linejoin="round"><rect x="7" y="5" width="86" height="90" rx="15" ry="15"/><path d="M27,15 L50,82 L73,15"/></svg>';
