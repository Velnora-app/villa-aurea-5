...
/**
 * Neutralise le rebond élastique natif de Safari iOS en haut/bas de page.
 * `overscroll-behavior:none` (velnora-shared.css) n'est pas respecté par
 * Safari sur le document racine — limitation WebKit connue, non résolue à
 * ce jour (Chrome/Firefox l'appliquent correctement, Safari non). Sans ce
 * correctif, un flick rapide qui atteint le bas de page avec encore de la
 * vitesse déclenche un rebond visible ("l'écran vibre"), perceptible
 * surtout sur les écrans de longueur moyenne où un flick a le temps de
 * prendre de la vitesse avant de toucher le bas (Accueil, Check-in/out,
 * Conciergerie, Guide pratique) — les écrans très courts n'ont pas assez
 * de distance pour que le doigt prenne de la vitesse, les écrans longs
 * laissent le temps au flick de ralentir naturellement avant le bas.
 *
 * On bloque UNIQUEMENT le dépassement vertical aux bornes du document, et
 * seulement quand le geste est majoritairement vertical, pour ne jamais
 * interférer avec les carrousels horizontaux (.strong-photos, .rec-tabs,
 * .x-tabs) ni avec le scroll interne du chat assistant, qui gèrent déjà
 * leur propre rebond en local.
 */
(function preventIOSRubberBand(){
  ...
  document.addEventListener('touchmove', function(e){
    if (e.touches.length !== 1) return;
    if (e.target.closest('.vln-chat-body, .strong-photos, .rec-tabs, .x-tabs')) return;

    const dx = e.touches[0].clientX - startX;
    const dy = e.touches[0].clientY - startY;
    if (Math.abs(dx) > Math.abs(dy)) return; // geste horizontal : on ne touche pas

    const doc = document.scrollingElement || document.documentElement;
    const atTop = doc.scrollTop <= 0;
    const atBottom = Math.ceil(doc.scrollTop + doc.clientHeight) >= doc.scrollHeight;
